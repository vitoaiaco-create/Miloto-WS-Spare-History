...
Rules should be stored in the `.cursor/rules` directory
Rule files must have a `.mdc` extension (e.g., `my-rule.mdc`)
To reference a file in your rule, use the format: `[filename.ext](mdc:filename.ext)` where the path is relative to the workspace root
Rules use Markdown format with special Cursor-specific extensions
Rules will be shown to the AI to help with codebase navigation and understanding
Metadata is stored in frontmatter and controls how the rule is used and must be formatted properly.
